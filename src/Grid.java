...
public class Grid {
  ...
  public void paint(Graphics g, Point mousePos) {
    for(int i=0; i<cells.length; i++) {
      for(int j=0; j<cells[i].length; j++) {
        Cell cellToPaint = cells[i][j];
        if (cellToPaint.getTemperature() > 0.0) {
            cellToPaint = new TemperatureDecorator(cellToPaint, cellToPaint.getTemperature());
        }
        cellToPaint.paint(g, mousePos);
      }
    }
  }
